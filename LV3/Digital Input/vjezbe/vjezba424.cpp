...
int main(void) {
	...
	while (1) {
		// ako je filtrirani pin PD0 u logickoj nuli (drzi tipkalo 1sec)
		if(filtered_pin_state(PD,PD0,false,1000) == false) {	
		...
		// ako je filtrirani pin PD1 u logickoj nuli (drzi tipkalo 1sec)
		if(filtered_pin_state(PD,PD1,false,1000) == false) {
			set_port(PORTB, PB5, 1);	// ukljuci crvenu LED diodu
		}
		else {
			set_port(PORTB, PB5, 0);	// inace je iskljuci
		}
		
		// ako je filtrirani pin PF6 u logickoj nuli (drzi tipkalo 1sec)
		if(filtered_pin_state(PF,PF6,false,1000) == false) {
			set_port(PORTB, PB6, 1);	// ukljuci crvenu LED diodu
		}
		else {
			set_port(PORTB, PB6, 0);	// inace je iskljuci
		}
		
		// ako je filtrirani pin PF7 u logickoj nuli (drzi tipkalo 1sec)
		if(filtered_pin_state(PF,PF7,false,1000) == false) {
			set_port(PORTB, PB7, 1);	// ukljuci crvenu LED diodu
		}
		else {
